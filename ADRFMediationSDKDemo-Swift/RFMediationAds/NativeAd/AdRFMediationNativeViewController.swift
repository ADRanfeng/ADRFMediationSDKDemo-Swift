//
//  ADRFMediationNativeViewController.swift
...
class AdRFMediationNativeViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, ADRFMediationSDKNativeAdDelegate {
    func adrf_nativeAdSucess(toLoad nativeAd: ADRFMediationSDKNativeAd, adViewArray: [UIView & ADRFMediationAdapterNativeAdViewDelegate]) {
        if adViewArray.count > 0{
            for item : UIView&ADRFMediationAdapterNativeAdViewDelegate in adViewArray {
                // 判断信息流广告是否为自渲染类型（可选实现） 可仿照所示样式demo实现 如无所需样式则需自行实现
                // 如果单纯只配置了模版信息流，那么不需要实现，如果配置了自渲染信息流，那么需要实现
                if item.renderType() == ADRFMediationAdapterRenderType.native {
                    //1、常规样式
...
//                    setUpUnifiedOnlyImageNativeAdView(adview: item)
    ...
    func adrf_nativeAdFail(toLoad nativeAd: ADRFMediationSDKNativeAd, errorModel: ADRFMediationAdapterErrorDefine) {
        self.mainTableView.mj_header?.endRefreshing()
        self.mainTableView.mj_footer?.endRefreshing()
    }
    
    func adrf_nativeAdViewRenderOrRegistSuccess(_ adView: UIView & ADRFMediationAdapterNativeAdViewDelegate) {
    ...
    func adrf_nativeAdViewRenderOrRegistFail(_ adView: UIView & ADRFMediationAdapterNativeAdViewDelegate) {
        
    }
    
    func adrf_nativeAdClicked(_ nativeAd: ADRFMediationSDKNativeAd, adView: UIView & ADRFMediationAdapterNativeAdViewDelegate) {
        
    }
    
    func adrf_nativeAdClose(_ nativeAd: ADRFMediationSDKNativeAd, adView: UIView & ADRFMediationAdapterNativeAdViewDelegate) {
    ...
    func adrf_nativeAdExposure(_ nativeAd: ADRFMediationSDKNativeAd, adView: UIView & ADRFMediationAdapterNativeAdViewDelegate) {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if item is ADRFMediationAdapterNativeAdViewDelegate {
            ...
            let tempItem = item as! UIView & ADRFMediationAdapterNativeAdViewDelegate
            if !tempItem.adrf_closeButtonExist() {
    ...
    var nativeAd : ADRFMediationSDKNativeAd!
    ...
    open var posid : String = "28192b9c7146c94711"
    ...
    func loadNativeAd() {
        if nativeAd == nil {
            self.nativeAd = ADRFMediationSDKNativeAd.init(adSize: CGSize.init(width: self.view.bounds.size.width, height: 0))
    ...
    // 1、常规样式
    func setUpUnifiedNativeAdView(adview : UIView & ADRFMediationAdapterNativeAdViewDelegate) {
        ...
        if adview.adrf_platform() != ADRFMediationAdapterPlatform.GDT {
            let logoImage = UIImageView()
            adview.addSubview(logoImage);
            adview.adrf_platformLogoImageDarkMode(false) { (image) in
        ...
        titleLabel.font = UIFont.adrf_PingFangMediumFont(14)
        titleLabel.textColor = UIColor.adrf_color(withHexString: "#333333")
        ...
        if adview.data?.shouldShowMediaView ?? false {
            let mediaView:UIView = adview.adrf_mediaView(forWidth: mainFrame.size.width) ?? UIView.init()
            mediaView.frame = mainFrame
            adview.addSubview(mediaView)
        } else {
            let imageView:UIImageView = UIImageView.init()
            imageView.backgroundColor = UIColor.adrf_color(withHexString: "#CCCCCC")
        ...
        adlabel.backgroundColor = UIColor.adrf_color(withHexString: "#CCCCCC")
        adlabel.textColor = UIColor.adrf_color(withHexString: "#FFFFFF")
        ...
        descLabel.textColor = UIColor.adrf_color(withHexString: "#333333")
    ...
    // 2、纯图
    func setUpUnifiedOnlyImageNativeAdView(adview : UIView & ADRFMediationAdapterNativeAdViewDelegate) {
        // 设计的adView实际大小，其中宽度和高度可以自己根据自己的需求设置
        let adWidth:CGFloat = self.view.bounds.size.width
    ...
    // 3、上图下文
    func setUpUnifiedTopImageNativeAdView(adview : UIView & ADRFMediationAdapterNativeAdViewDelegate) {
        // 设计的adView实际大小，其中宽度和高度可以自己根据自己的需求设置
        let adWidth:CGFloat = self.view.bounds.size.width
    ...
    func cleanAllAd() {
        if self.dataArray.count > 0 {
            for item : Any in self.dataArray {
                if item is ADRFMediationAdapterNativeAdViewDelegate {
                    let itemObj = item as? ADRFMediationAdapterNativeAdViewDelegate
                    if itemObj != nil {
                        itemObj?.adrf_unRegistView()
                    }
                }
    ...
    func getCloseButtonWithAdItem(item:UIView & ADRFMediationAdapterNativeAdViewDelegate)->UIButton{
        let closeButton = UIButton()
        //按钮位置根据需求自行设置
        closeButton.frame = CGRect(x:item.mj_w-44, y:0, width:44, height:44)
        closeButton.setImage(UIImage(named: "close"), for: .normal)
        closeButton.addTarget(item, action: #selector(item.adrf_close), for: .touchUpInside)
        return closeButton
    }
