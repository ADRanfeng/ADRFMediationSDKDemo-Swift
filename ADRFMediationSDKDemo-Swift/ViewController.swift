...
class ViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    private var dataArray: Array<String> = ["开屏广告","信息流广告","Banner横幅广告","激励视频","插屏广告"]
    private let tableViewCellID = "SimpleTableIdentifier"
    
    override func viewDidLoad() {
        super.viewDidLoad()
        // [NSString stringWithFormat:@"ADRFMediationSDK-Demo-v%@",[ADRFMediationSDK getSDKVersion]]
        self.title = NSString.init(format: "然峰 广告聚合SDK Demo") as String
...
//        setBtn.setTitle("设置", for: .normal)
    ...
    func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        ...
        label.text = ADRFMediationSDK.getVersion()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        labTitle.font = UIFont.adrf_PingFangRegularFont(14)
        labTitle.backgroundColor = UIColor.white
        labTitle.textAlignment = NSTextAlignment.center
        labTitle.tag = 999
        labTitle.textColor = UIColor.adrf_color(withHexString: "#666666")
        labTitle.text = title
        ...
        labTitle.layer.borderColor = UIColor.adrf_color(withHexString: "#E5E5EA")?.cgColor
        labTitle.layer.shadowColor = UIColor.adrf_color(withHexString: "#000000", alphaComponent: 0.1)?.cgColor
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case 0:
            self.navigationController?.pushViewController(ADRFMediationSplashViewController.init(), animated: true)
            break
        case 1:
            self.navigationController?.pushViewController(AdRFMediationNativeViewController.init(), animated: true)
            break
        case 2:
            self.navigationController?.pushViewController(AdRFMediationBannerViewController.init(), animated: true)
            break
        case 3:
            self.navigationController?.pushViewController(AdRFMediationRewardViewController.init(), animated: true)
            break
        case 4:
            self.navigationController?.pushViewController(AdRFMediationInterstitialViewController.init(), animated: true)
            break
        default:
            break
        }
